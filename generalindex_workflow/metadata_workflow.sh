...
psql -d generalindex_metadata -f ~/generalindex_workflow/generalindex_workflow/drop_tables_query.sql #drop_tables_query.sql contains a SQL statement that drops all tables like doc_meta_*
end=`date +%s`
echo execution time was `expr $end - $start` seconds.
...
start=`date +%s`
psql -d generalindex_metadata -f ~/generalindex_workflow/generalindex_workflow/drop_tables_query.sql
end=`date +%s`
echo execution time was `expr $end - $start` seconds.
